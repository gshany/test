...
//分页配置
var config = {
    limit: 2, //页容量：每页显示多少条数据
    total: datas.length, //总数据量
    page: 6, //当前是第几页
    panelNumber: 16 //数字页码最多出现的个数
}

/**
 * 根据提供的电影数组，生成响应的html元素，加入到电影容器中
 */
function setMovieHtml(movies) {
    var html = "";
    for (var i = 0; i < movies.length; i++) {
        var m = movies[i]; //拿到一个电影对象
        html += `<div class="data">
                        <div class="poster">
                        ...
                        <div class="words">
                            ...
                            <div class="attach">
                                <span>英文名：${m.ename}</span>
                                <span>类型：${m.type}</span>
                                <br>
                                <span>上映地区：${m.area}</span>
                                <span>上映时间：${m.upDate}</span>
                                <span>时长：${m.time}</span>
                            </div>
                            <div class="desc">
                                ${m.intro}
    ...
    document.getElementById("divMovies").innerHTML = html;
}
...
function loadPageData() {
    ...
    //从所有数据中取出这一页需要的数据，展示出来
    var movies = datas.slice((page - 1) * config.limit, page * config.limit); // slice截取个数，返回一个新数组，截取不包含第二个参数  (0,2) (2,4) (4,6).....
    setMovieHtml(movies)
}
...
function setPager() {
    //计算总页数
    var totalPage = Math.ceil(config.total / config.limit);
    var divPager = document.getElementById("pager");
    divPager.innerHTML = "";                          //清空 在重新加载页码
    //首页和上一页
    if (config.page === 1) {
        //当前是第一页         首页和上一页是没有点击事件
        createPager("首页", "first disabled");
        createPager("上一页", "prev disabled");
    }
    else {
        //当前不是第一页       有点击事件
        var first = createPager("首页", "first");
        var prev = createPager("上一页", "prev");
        ...
        prev.onclick = function () {
            ...
            // load(config.page -1 );         //不能写 --  不调用 --是不会变的
        }
    }

    //数字页码
    var min = config.page - Math.floor(config.panelNumber / 2); //最小页码
    if (min < 1) {
        min = 1; //最小页码至少为1
    }
    ...
    if (max > totalPage) {
        ...
        // min = max - config.panelNumber + 1; //重新计算最小页码
        // if (min < 1) {
        //     min = 1;
        // }
    }
    for (var i = min; i <= max; i++) {
        if (i === config.page) {
            ...
            a.onclick = function () {
                var page = parseInt(this.innerText) //拿到a元素内部的文本
                load(page)
            }
        }
    }
    ...
    if (config.page === totalPage) {
        ...
        //当前不是最后一页
        var next = createPager("下一页", "first");
        var last = createPager("尾页", "prev");
        next.onclick = function () {
            load(config.page +1);   //不能写 ++   但是++可以写前面
        }
    ...
    span.innerHTML = `<i class="current">${config.page}</i> 
    / 
    ...
    /**
     * 加载指定页码的数据，以及设置页码区域，他是loadPageData和setPager的组合
     * @param {*} page 
     */
    function load(page) {
        config.page = page;
